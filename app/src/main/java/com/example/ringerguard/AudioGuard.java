...
public final class AudioGuard {
     ...
     *
     * 注意：
     * 为了兼容 iQOO 静音，本版本会在铃声音量为 0 时恢复铃声音量。
     * 不修改媒体音量、闹钟音量。
     */
    public static boolean enforce(Context context) {
        ...
        if (mode != -1 && mode != AudioManager.RINGER_MODE_NORMAL) {
            changed |= setRingerModeNormalSafe(am);
        }

        /*
         * iQOO 上有些情况不是改 ringer mode，
         * 而是把铃声流 / 通知流 mute。
         *
        ...
        /*
         * 部分系统 unmute 后会重新计算 ringer mode，
         * 所以再确认一次。
         */
        mode = getRingerModeSafe(am);
         ...
         * iQOO / OriginOS 常见情况：
         *
         * getRingerMode() 已经是 NORMAL，
         * 但是铃声音量实际是 0，
         * 来电仍然不响。
         *
         * 所以这里专门处理 ring volume == 0。
         */
        ...
        /*
         * 设置音量后，部分系统可能又把 ringer mode 带到震动/静音，
         * 最后再确认一次。
         */
        mode = getRingerModeSafe(am);
    ...
    public static String interruptionFilterToText(int filter) {
        if (filter == NotificationManager.INTERRUPTION_FILTER_ALL) {
            ...
            return "勿扰：全部拦截";
        } else if (filter == NotificationManager.INTERRUPTION_FILTER_UNKNOWN) {
            return "未知";
        } else {
            return "未知";
        }
